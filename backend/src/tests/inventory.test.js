...
Module._load = function (request, parent, isMain) {
  // Resolve the request to an absolute path the same way Node would, so both
  // the literal '../db' and the already-resolved cache key match.
...
test('deductStock reduces stock_quantity AND reserved_stock for every item', () => {
  ...
  return inventory.deductStock('o1').then(() => {
    // 1 order_items lookup + 2 product updates
